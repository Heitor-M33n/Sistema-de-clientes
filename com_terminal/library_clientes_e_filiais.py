...
def visualizar_dados(c: list, v: list[str], f: list[str]): #de modo cru, está concluída
    v.insert(0, 'filial')
    print(v)

    for i in f: #i = filial (str)
        for y in c[f.index(i)]: #y = linha da filial (list)
            y.insert(0, i[0:-4])
            print(y)

    v.pop(0)

def visualizar_dados_filial(c: list, v: list[str], f: list[str]): #de modo cru, está concluída
...
def adicionar_filial(c: list[list], f: list[str]) -> list[str]: #completa, precisa de testes
...
def remover_filial(f: list[str], c: list[list]): #completa, falta o mover
    f_sem_csv = [x[0:-4] for x in f]
    l.print_em_ordem_numerado(f_sem_csv)
